...
class SSTEmotionNet(nn.Module):
    ...
    def __init__(
        ...
        assert (
            grid_size[0] >= 16 and grid_size[1] >= 16
        ), "The height and width of the grid must be greater than or equal to 16. Please upsample the EEG grid."

        self.spatial_spectral = DenseNet3D(
            grid_size=grid_size,
        ...
        self.spatial_temporal = DenseNet3D(
            grid_size=grid_size,
    ...
    def get_feature_dims(self):
        mock_eeg_s = torch.randn(
            2, self.grid_size[0], self.grid_size[1], self.spectral_in_channels
        )
        mock_eeg_t = torch.randn(
            2, self.grid_size[0], self.grid_size[1], self.temporal_in_channels
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        """
        assert (
            x.shape[1] == (self.spectral_in_channels + self.temporal_in_channels)
    ...
    def __init__(
        ...
        if subsample_initial_block:
            ...
            layers.append(
                (
                    "maxpool",
                    nn.MaxPool3d((2, 2, 2), stride=(2, 2, 2), padding=(0, 0, 1)),
                )
            )
        ...
        for block_idx in range(num_dense_block - 1):
            layers.append(
                Attention(
                    grid_size=(grid_height, grid_width), in_channels=grid_channels
                )
            )

            layers.append(
                DenseBlock(
                    num_layers[block_idx],
                    num_filters,
                    growth_rate,
            ...
            layers.append(Transition(num_filters, num_filters, compression=compression))
        ...
        layers.append(
            Attention(grid_size=(grid_height, grid_width), in_channels=grid_channels)
        )
        layers.append(
            DenseBlock(
                num_layers[block_idx],
        ...
        final_layers.append(nn.AvgPool3d((grid_height, grid_width, grid_channels)))
        self.final_layers = nn.ModuleList(final_layers)

    def get_feature_dims(self):
        mock_eeg = torch.randn(
            2, self.grid_size[0], self.grid_size[1], self.in_channels
        )
...
class DenseBlock(nn.Module):
    def __init__(
        self, num_layers, num_filters, growth_rate, bottleneck=False, dropout=None
    ):
...
class ConvBlock(nn.Module):
    def __init__(
        self, input_channel, num_filters, bottleneck=False, dropout=None, conv1x1=True
    ):
        ...
        if bottleneck:
            ...
            layers.append(
                nn.Conv3d(
                    input_channel, inter_channel, (1, 1, 1), padding=0, bias=False
                )
            )
        ...
        layers.append(
            nn.Conv3d(
                inter_channel, num_filters, (3, 3, 1), padding=(1, 1, 0), bias=False
            )
        )
        ...
        if conv1x1:
            layers.append(
                nn.Conv3d(
                    num_filters, num_filters, (1, 1, 1), padding=(0, 0, 0), bias=False
                )
            )

        layers.append(
            nn.Conv3d(
                num_filters, num_filters, (1, 1, 3), padding=(0, 0, 1), bias=False
            )
        )
...
class Attention(nn.Module):
    def __init__(self, grid_size, in_channels):
        ...
        self.temporal_pool = nn.AvgPool3d(kernel_size=[grid_size[0], grid_size[1], 1])
